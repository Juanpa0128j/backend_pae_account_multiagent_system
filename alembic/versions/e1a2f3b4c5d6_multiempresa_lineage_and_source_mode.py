...
def upgrade() -> None:
    ...
    for table in ("transactions_pending", "transactions_posted", "journal_entry_lines"):
        if not _column_exists(table, "company_nit"):
            op.add_column(
                table,
                sa.Column("company_nit", sa.String(length=20), nullable=True,
    ...
    for idx_name, table, columns in (
        ("ix_transactions_pending_company_nit", "transactions_pending", ["company_nit"]),
        ...
        ("ix_financial_statements_entity_type_period", "financial_statements",
    ...
    if not _table_exists("financial_statement_lineage"):
        ...
        op.create_index("ix_financial_statement_lineage_id",
                        "financial_statement_lineage", ["id"], unique=False)
        op.create_index("ix_financial_statement_lineage_target_statement_id",
                        "financial_statement_lineage", ["target_statement_id"], unique=False)
        op.create_index("ix_financial_statement_lineage_source_statement_id",
                        "financial_statement_lineage", ["source_statement_id"], unique=False)
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index("ix_financial_statement_lineage_source_statement_id",
                  table_name="financial_statement_lineage")
    op.drop_index("ix_financial_statement_lineage_target_statement_id",
                  table_name="financial_statement_lineage")
    op.drop_index("ix_financial_statement_lineage_id",
                  table_name="financial_statement_lineage")

...
def _state(asientos=None, pending_transaction_id="pending_001", fecha="2026-01-15"):
    raw_transactions = [{"fecha": fecha}] if fecha else []
    return {
        "mode": "process",
...
class TestPrePersistAuditor:
    ...
    def test_missing_fecha_emits_blocker(self):
        state = _state(
            asientos=[
                {"tipo_movimiento": "debito", "valor": "1000", "cuenta_puc": "511005"},
                {"tipo_movimiento": "credito", "valor": "1000", "cuenta_puc": "220505"},
        ...
        report = pre_persist_auditor.run(state)
        assert report.approved is False
        finding = next(f for f in report.findings if f.rule_id == "ING-FECHA-MISSING")
        assert finding.severity == Severity.BLOCKER

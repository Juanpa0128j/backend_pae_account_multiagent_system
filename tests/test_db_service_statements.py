...
def test_financial_statements_exist_returns_true_when_present():
    ...
    db = MagicMock()
    # financial_statements_exist chains 4 .filter() calls: entity_nit, period_end>=, period_end<=, statement_type.in_()
    mock_query = db.query.return_value.filter.return_value.filter.return_value.filter.return_value.filter.return_value

...
def bank_statement(text: str, *, correction_feedback: str | None = None) -> str:
    instructions = (
        ...
        "REGLA PRINCIPAL — `movements`: INCLUYE TODA fila que tenga FECHA + VALOR numérico, sin excepción. "
        "Si dudas, INCLÚYELA. Mejor un movement extra que perder data contable.\n\n"
        "Tipos de movimientos REALES que DEBES capturar (no son footers, son transacciones del libro auxiliar "
        "del banco — cada uno mueve el saldo):\n"
        "  • ABONO INTERESES AHORROS / ABONO INTERESES (rendimientos)\n"
        "  • PAGO DE PROV ... (pagos a proveedores con nombre)\n"
        "  • PAGO DE TERC ... (pagos a terceros con nombre)\n"
        "  • PAGO PSE ... (pagos vía PSE: proveedores, impuestos, servicios, CARTERA COLECTIVA, ENLACE OPERATIVO, IMPUESTO DIAN, etc.)\n"
        "  • TRANSFERENCIA CTA SUC VIRTUAL (transferencias entrantes/salientes)\n"
        "  • TRANSFERENCIA ACH / SPI / BANCAMÍA / etc.\n"
        "  • IMPTO GOBIERNO 4X1000 / GMF (impuesto a transacciones financieras — siempre un movimiento real)\n"
        "  • CUOTA MANEJO TRJ DEB / TRJ CRED (comisiones bancarias)\n"
        "  • AJUSTE INTERES AHORROS DB / CR (ajustes técnicos)\n"
        "  • RETIRO CAJERO / DEPOSITO / CONSIGNACIÓN\n"
        "  • Cualquier débito o crédito con fecha + monto (aunque parezca automático/operativo).\n\n"
        "ÚNICAS filas a DESCARTAR (NO van en `movements`):\n"
        "  • Encabezados de columnas: 'FECHA | DESCRIPCION | SUCURSAL | DCTO | VALOR | SALDO'.\n"
        "  • Bloque RESUMEN del top del extracto: SALDO ANTERIOR, SALDO ACTUAL, SALDO PROMEDIO, "
        "TOTAL ABONOS, TOTAL CARGOS, VALOR INTERESES PAGADOS, CUENTAS X COBRAR, RETEFUENTE (estos van "
        "en los campos `saldo_inicial`/`saldo_final`/`resumen`, NO en `movements`).\n"
        "  • Fila 'FIN ESTADO DE CUENTA' (sin valor numérico).\n"
        "  • Publicidad/promociones del banco sin fecha+valor (URLs, ofertas, fotos).\n\n"
        "REGLA DE ORO: si la fila tiene FECHA + VALOR numérico (positivo o negativo) → INCLÚYELA. "
        "El campo `descripcion` puede ser vacío si el texto está cortado/ilegible, pero el movement debe "
        "persistirse. NO truncar la lista; extraer TODOS los movements de TODAS las páginas."
    )
    return _build_prompt(instructions, text, correction_feedback=correction_feedback)
...
def auxiliary_ledger(text: str, *, correction_feedback: str | None = None) -> str:
    instructions = (
        ...
        "Extrae obligatoriamente: entidad, cuenta principal PUC (código y nombre), período "
        "(periodo_inicio y periodo_fin en formato YYYY-MM-DD), **periodicidad** "
        "('mensual' si cubre un mes, 'trimestral' si un trimestre, 'anual' si "
        "es el cierre del ejercicio o 'libro auxiliar consolidado del año', "
        "'personalizado' para otros rangos), saldo inicial, "
...
def balance_general(text: str, *, correction_feedback: str | None = None) -> str:
    instructions = (
        ...
        "Extrae obligatoriamente: entidad (NIT, razón social), fecha de corte (periodo_fin), "
        "**periodicidad** ('mensual' si dice 'al 31 de enero/febrero/...', "
        "'anual' si dice 'al 31 de diciembre', 'cierre del ejercicio', 'estados financieros anuales', "
        "'trimestral' si es cierre trimestral, 'personalizado' otros casos), marco normativo "
...
def estado_resultados(text: str, *, correction_feedback: str | None = None) -> str:
    instructions = (
        ...
        "Extrae obligatoriamente: entidad (NIT, razón social), período (periodo_inicio y "
        "periodo_fin), **periodicidad** ('anual' si dice 'año terminado en diciembre', "
        "'estados financieros anuales', 'del 1 de enero al 31 de diciembre'; 'mensual' si "
        "cubre un mes; 'trimestral' si cubre un trimestre; 'personalizado' otros casos), "
        "marco normativo, "
...
def nomina(text: str, *, correction_feedback: str | None = None) -> str:
    instructions = (
        ...
        "NÓMINA.\n\n"
        "Extrae obligatoriamente: empresa (NIT, razón social), período de nómina (inicio y fin en formato YYYY-MM-DD), "
        "para cada empleado: nombre, cédula, cargo, salario básico, días trabajados, total devengado, deducciones "
        "(salud empleado 4%, pensión empleado 4%, retención en la fuente), otras deducciones, total deducciones, neto a pagar. "
        "También extrae los totales consolidados y los aportes patronales (salud 8.5%, pensión 12%, ARL, SENA, ICBF, "
        "caja de compensación).\n\n"
        "REGLAS OBLIGATORIAS PARA FECHAS (MUY IMPORTANTE):\n"
        "- Si dice 'Primera Quincena' del mes M/AAAA → periodo_inicio=AAAA-MM-01, periodo_fin=AAAA-MM-15\n"
        "- Si dice 'Segunda Quincena' del mes M/AAAA → periodo_inicio=AAAA-MM-16, periodo_fin=último día del mes\n"
        "- Si dice 'Mes M/AAAA' sin quincena → periodo_inicio=AAAA-MM-01, periodo_fin=último día del mes\n"
        "- Siempre convierte referencias como '1/2026' a año 2026 mes 1 = enero\n"
        "- NUNCA dejes periodo_inicio ni periodo_fin en blanco si hay información de período en el documento\n\n"
        "MUY IMPORTANTE — TABLA DE ASIENTOS PRE-ARMADOS:\n"
...
def planilla_seg_social(text: str, *, correction_feedback: str | None = None) -> str:
    instructions = (
        ...
        "MAPEO DE ADMINISTRADORAS POR RUBRO (CRÍTICO — Colombia):\n"
        "Los comprobantes de pago de operadores (Enlace Operativo / SuAporte / Aportes en Línea / Mi Planilla) "
        "incluyen una tabla con las administradoras y su valor. Mapea cada fila a su rubro POR EL NOMBRE de la "
        "administradora, NO por la posición de la columna (las columnas suelen estar desfasadas):\n"
        "- PENSIÓN (suma a total_pension): COLPENSIONES, PORVENIR, PROTECCIÓN, COLFONDOS, OLD MUTUAL, SKANDIA, "
        "y cualquier 'AFP' o 'Fondo de Pensiones'.\n"
        "- SALUD (suma a total_salud): cualquier nombre que contenga 'EPS' o sea una EPS conocida (NUEVA EPS, "
        "SANITAS, SURA EPS, COMPENSAR EPS, SALUD TOTAL, COOMEVA, FAMISANAR, ALIANSALUD, MUTUAL SER, "
        "CAPITAL SALUD, ASMET SALUD, EMSSANAR, COOSALUD).\n"
        "- ARL (suma a total_arl): cualquier nombre con 'ARL' o aseguradora de riesgos (ARL SURA, POSITIVA, "
        "COLMENA SEGUROS, AXA COLPATRIA ARL, BOLÍVAR, LIBERTY ARL, EQUIDAD SEGUROS).\n"
        "- CAJA (suma a total_caja): 'CAJA DE COMPENSACIÓN FAMILIAR' o cajas conocidas (COMFAMA, COMPENSAR, "
        "COMFANDI, CAFAM, COMFENALCO, COMFAMILIAR, CAJASAN, CAJACOPI, CAJAMAG).\n"
        "- PARAFISCALES (suma a total_parafiscales): SENA, ICBF.\n\n"
        "IMPORTANTE:\n"
        "- Lee TODAS las filas de la tabla de administradoras, no solo las primeras ni las últimas.\n"
        "- Usa el 'Valor sin Mora' (no 'Total Intereses Mora') de cada fila.\n"
        "- La suma de los totales por rubro debe ser igual a total_a_pagar. Si no cuadra, revisa que "
        "no hayas omitido ninguna fila ni confundido columnas.\n"
        "- Si solo encuentras el período mensual y no una fecha específica de pago, usa el primer "

...
_DOC_GUIDANCE: dict[str, str] = {
    ...
    "nota_credito": (
        "REGLA NOTA CREDITO: una nota crédito ajusta una factura previa y puede ser de "
        "VENTA o de COMPRA. Decide la dirección comparando el emisor con la empresa:\n"
        "- NC de VENTA (la empresa EMITIÓ la factura/nota): asiento de reversión de "
        "ingreso → DÉBITO a la cuenta de ingreso específica (4135/4170/4175 según concepto) "
        "y CRÉDITO a cuentas por cobrar (130505).\n"
        "- NC de COMPRA (la empresa RECIBIÓ la nota de un proveedor): reversa el costo/gasto "
        "→ DÉBITO a cuentas por pagar (2205) y CRÉDITO a la cuenta de gasto/costo o de "
        "inventario originalmente registrada (5xxx/6xxx/14xx).\n"
        "NUNCA uses '4xxx' o '5xxx' literal. Referencia el número de factura asociada en "
        "descripcion_general. NO dupliques impuestos — el agente tributario los maneja."
    ),
    "nota_debito": (
        "REGLA NOTA DEBITO: una nota débito ajusta (aumenta) una factura previa y puede ser "
        "de VENTA o de COMPRA. Decide la dirección comparando el emisor con la empresa:\n"
        "- ND de VENTA (la empresa EMITIÓ la nota): mayor valor del ingreso → DÉBITO a "
        "cuentas por cobrar (130505) y CRÉDITO a la cuenta de ingreso específica "
        "(4135/4170/4175). Si son intereses moratorios, acredita ingresos financieros "
        "(clase 42, p. ej. 4210).\n"
        "- ND de COMPRA (la empresa RECIBIÓ la nota de un proveedor): mayor valor del "
        "costo/gasto → DÉBITO a la cuenta de gasto/costo/inventario (5xxx/6xxx/14xx) y "
        "CRÉDITO a cuentas por pagar (2205).\n"
        "NUNCA uses '4xxx', '5xxx' o '13xxxx' literal. Referencia el número de factura "
        "asociada en descripcion_general. NO dupliques impuestos — el agente tributario los maneja."
    ),
    "comprobante_egreso": (

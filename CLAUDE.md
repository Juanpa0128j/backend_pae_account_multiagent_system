...
- **Parser cache key includes `parser_mode`:** `ingest_agent.py:_parse_single_file` caches LlamaParse output as `<sha256>.<mode>.md` (was `<sha256>.md`). Switching fast↔premium↔gpt4o now invalidates the cache. Old `<hash>.md` files become orphans and are auto-pruned by TTL.
